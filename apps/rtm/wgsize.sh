...
do
  ...
  do
    ...
    do
      ...
      echo "wg size test ${k} ${j} ${i}" >> rtm_"$ACCEL"_wgtest3d
      output=$(stdbuf -o0 ./rtm_"$ACCEL" -sizex=320 -sizey=320 -sizez=320 -iters=20 -OPS_DIAGS=2 OPS_SYCL_DEVICE=$SYCL_DEVICE -gpudirect OPS_BLOCK_SIZE_X=${k} OPS_BLOCK_SIZE_Z=${j} OPS_BLOCK_SIZE_Y=${i})
      echo "$output" >> rtm_"$ACCEL"_wgtest3d
