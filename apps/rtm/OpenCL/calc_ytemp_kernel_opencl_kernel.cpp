...
//
    ...
    static bool isbuilt_calc_ytemp_kernel = false;

    void buildOpenCLKernels_calc_ytemp_kernel(OPS_instance *instance, int xdim2, int ydim2, int zdim2, int xdim3, int ydim3, int zdim3, int xdim4, int ydim4, int zdim4) {

      //int ocl_fma = OCL_FMA;
      if(!isbuilt_calc_ytemp_kernel) {
        buildOpenCLKernels(instance);
        //clSafeCall( clUnloadCompiler() );
        cl_int ret;
        char* source_filename[1] = {(char*)"./OpenCL/calc_ytemp_kernel.cl"};
        ...
        instance->ostream() <<"Compiling calc_ytemp_kernel "<<OCL_FMA<<" source -- start \n";
          ...
          if (pPath!=NULL)
            if(OCL_FMA)
              sprintf(buildOpts,"-cl-mad-enable -DOCL_FMA -I%s/include -DOPS_WARPSIZE=%d  -Dxdim2_calc_ytemp_kernel=%d  -Dydim2_calc_ytemp_kernel=%d  -Dzdim2_calc_ytemp_kernel=%d  -Dxdim3_calc_ytemp_kernel=%d  -Dydim3_calc_ytemp_kernel=%d  -Dzdim3_calc_ytemp_kernel=%d  -Dxdim4_calc_ytemp_kernel=%d  -Dydim4_calc_ytemp_kernel=%d  -Dzdim4_calc_ytemp_kernel=%d ", pPath, 32,xdim2,ydim2,zdim2,xdim3,ydim3,zdim3,xdim4,ydim4,zdim4);
            else
          ...
          instance->ostream() << "compiling calc_ytemp_kernel -- done\n";

        // Create the OpenCL kernel
        instance->opencl_instance->OPS_opencl_core.kernel[2] = clCreateKernel(instance->opencl_instance->OPS_opencl_core.program, "ops_calc_ytemp_kernel", &ret);
        clSafeCall( ret );

        isbuilt_calc_ytemp_kernel = true;
        free(source_str[0]);
      }
...
void ops_par_loop_calc_ytemp_kernel(char const *name, ops_block block, int dim, int* range,
 ...
 ops_arg arg4) {
  ...
  buildOpenCLKernels_calc_ytemp_kernel(block->instance,
  xdim2,ydim2,zdim2,xdim3,ydim3,zdim3,xdim4,ydim4,zdim4);
  ...
  if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
    ...
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 0, sizeof(cl_float), (void*) arg1.data ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 1, sizeof(cl_mem), (void*) &arg2.data_d ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 2, sizeof(cl_mem), (void*) &arg3.data_d ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 3, sizeof(cl_mem), (void*) &arg4.data_d ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 4, sizeof(cl_int), (void*) &base2 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 5, sizeof(cl_int), (void*) &base3 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 6, sizeof(cl_int), (void*) &base4 ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 7, sizeof(cl_int), (void*) &arg_idx[0] ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 8, sizeof(cl_int), (void*) &arg_idx[1] ));
    clSafeCall( clSetKernelArg(block->instance->opencl_instance->OPS_opencl_core.kernel[2], 9, sizeof(cl_int), (void*) &arg_idx[2] ));
