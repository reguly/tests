...
void runCUDA(float* results)
{
  ...
  hipSetDevice(0);
  hipMalloc((void**)&_cuda.d_protein, params.natpro*sizeof(Atom));
  hipDeviceSynchronize();
  hipMemcpy(_cuda.d_protein, params.protein, params.natpro*sizeof(Atom), hipMemcpyHostToDevice);
  hipDeviceSynchronize();

  hipMalloc((void**)&_cuda.d_ligand, params.natlig*sizeof(Atom));
  hipDeviceSynchronize();
  hipMemcpy(_cuda.d_ligand, params.ligand, params.natlig*sizeof(Atom), hipMemcpyHostToDevice);
  hipDeviceSynchronize();

  hipMalloc((void**)&_cuda.d_forcefield, params.ntypes*sizeof(FFParams));
  hipDeviceSynchronize();
  hipMemcpy(_cuda.d_forcefield, params.forcefield, params.ntypes*sizeof(FFParams), hipMemcpyHostToDevice);
  hipDeviceSynchronize();

  hipMalloc((void**)&_cuda.d_results, params.nposes*sizeof(float));
  hipDeviceSynchronize();

  for(int ii = 0; ii < 6; ++ii)
  {
    hipMalloc((void**)&_cuda.d_poses[ii], params.nposes*sizeof(float));
    hipDeviceSynchronize();
    hipMemcpy(_cuda.d_poses[ii], params.poses[ii], params.nposes*sizeof(float), hipMemcpyHostToDevice);
    hipDeviceSynchronize();
  }
  ...
  hipMemcpy(results, _cuda.d_results, params.nposes*sizeof(float), hipMemcpyDeviceToHost);
  hipDeviceSynchronize();
