...
{ 
  ...
  BEGIN_PROFILING("Compute fluxes x")
  q.submit([&] (handler &cgh) {
    #ifdef NDRANGE
    cgh.parallel_for<class compute_flux_x>(nd_range<3>(flux_gws, flux_lws), [=] (nd_item<3> item) {
    #else
    cgh.parallel_for<class compute_flux_x>(range<3>(1,nz,nx+1), [=] (item<3> item) {
    #endif
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class compute_tend_x>(range<3>(NUM_VARS,nz,nx), [=] (item<3> item) {
    #endif
  ...
  END_PROFILING("Compute tendencies x")
}
    ...
    queue &q ) 
{
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class compute_flux_z>(range<3>(1,nz+1,nx), [=] (item<3> item) {
    #endif
      int k = item.get_global_id(1);
      int i = item.get_global_id(2);
      double stencil[4], d3_vals[NUM_VARS], vals[NUM_VARS];
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class compute_tend_z>(range<3>(NUM_VARS,nz,nx), [=] (item<3> item) {
    #endif
      int ll = item.get_global_id(0);
      int k = item.get_global_id(1);
      int i = item.get_global_id(2);
  ...
  END_PROFILING("Compute tendencies z")
}
    ...
    queue &q ) 
{
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class pack_send_buf>(range<3>(NUM_VARS,nz,hs), [=] (item<3> item) {
    #endif
      int ll = item.get_global_id(0);
      int k = item.get_global_id(1);
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class unpack_recv_buf>(range<3>(NUM_VARS,nz,hs), [=] (item<3> item) {
    #endif
      int ll = item.get_global_id(0);
      int k = item.get_global_id(1);
      int s = item.get_global_id(2);
  ...
  if (data_spec_int == DATA_SPEC_INJECTION) {
    if (myrank == 0) {
      ...
      q.submit([&] (handler &cgh) {
        ...
        #else
        cgh.parallel_for<class update_state_x>(range<3>(1,nz,hs), [=] (item<3> item) {
        #endif
          int k = item.get_global_id(1);
          int i = item.get_global_id(2);
          if (i < hs && k < nz) { 
    ...
    queue &q ) 
{
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class update_state_z>(range<3>(1,NUM_VARS,nx+2*hs), [=] (item<3> item) {
    #endif
    ...
    queue &q ) 
{
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class reduce>(range<3>(gws), [=] (item<3> item) {
    #endif
      int k = item.get_global_id(1);
      int i = item.get_global_id(2);
      if (k < nz && i < nx) {
    ...
    queue &q  ) 
{
  ...
  q.submit([&] (handler &cgh) {
    ...
    #else
    cgh.parallel_for<class update_fluid_state>(range<3>(NUM_VARS, nz, nx), [=] (item<3> item) {
    #endif
      int ll = item.get_global_id(0);
      int k = item.get_global_id(1);
      int i = item.get_global_id(2);
  ...
  END_PROFILING("Apply tendencies")
}
...
int main(int argc, char **argv) {
  ...
  while (etime < sim_time) {
    ...
    etime = etime + dt;
    if (masterproc) {
      printf("Step %d of %d :: Time = %lf / %lf\n",step,(int)(sim_time/dt),etime,sim_time);
    }
  }
