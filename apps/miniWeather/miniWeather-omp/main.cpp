...
#else
#pragma omp parallel for collapse(2) private(inds,indt)
#endif
...
#else
#pragma omp parallel for collapse(1) private(ll,s,inds,stencil,vals,d3_vals,r,u,w,t,p)
#endif
  for (k=0; k<nz; k++) {
#pragma omp simd
    for (i=0; i<nx+1; i++) {
...
#else
#pragma omp parallel for collapse(2) private(indt,indf1,indf2)
#endif
  for (ll=0; ll<NUM_VARS; ll++) {
    for (k=0; k<nz; k++) {
      #pragma omp simd
...
#else
#pragma omp parallel for collapse(2) private(x,xloc,mnt_deriv)
#endif
  for (ll=0; ll<NUM_VARS; ll++) {
    for (i=0; i<nx+2*hs; i++) {
      if (ll == ID_WMOM) {
...
#pragma omp target map(to: state[0:(nz+2*hs)*(nx+2*hs)*NUM_VARS]) map(tofrom: mass, te) 
  ...
  #else
  #pragma omp parallel for collapse(2) reduction(+:mass,te)
  #endif
  for (int k=0; k<nz; k++) {
    for (int i=0; i<nx; i++) {
