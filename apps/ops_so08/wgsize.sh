...
do
  ...
  do
    ...
    do
      ...
      output=$(stdbuf -o0 ./wave-propagation_"$ACCEL" 1000 1000 1000 -OPS_DIAGS=2 OPS_SYCL_DEVICE=$SYCL_DEVICE -gpudirect OPS_BLOCK_SIZE_X=${k} OPS_BLOCK_SIZE_Z=${j} OPS_BLOCK_SIZE_Y=${i})
      echo "$output" >> so08_"$ACCEL"_wgtest3d
      # Extract the time from the output and compare with the lowest time
      time=$(echo "$output" | grep "Wave propagation time:" | awk '{print $4}')
