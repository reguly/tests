...
do
	if [ -n "$CPUTEST" ]; then
	OMP_NUM_THREADS=1 mpirun -np $physical_cores $bind_core ./opensbli_mpi 320 320 320 -OPS_DIAGS=2>> sn_"$precsuffix"_mpi"$physical_cores"_icc_diag2
  if [ "$logical_cores" -ne "$physical_cores" ]; then
	OMP_NUM_THREADS=1 mpirun -np $logical_cores $bind_hwthread ./opensbli_mpi 320 320 320 -OPS_DIAGS=2>> sn_"$precsuffix"_mpi"$logical_cores"_icc_diag2
	...
	for i in "${array[@]}"
	do
	OMP_NUM_THREADS=1 mpirun -np $physical_cores $bind_core ./opensbli_mpi_tiled 320 320 320 -OPS_DIAGS=2 OPS_TILING_MAXDEPTH=6 OPS_CACHE_SIZE=$i >> sn_"$precsuffix"_mpi"$physical_cores"_tiled_"$i"MB_icc_diag2
	if [ "$logical_cores" -ne "$physical_cores" ]; then
	OMP_NUM_THREADS=1 mpirun -np $logical_cores $bind_hwthread ./opensbli_mpi_tiled 320 320 320 -OPS_DIAGS=2 OPS_TILING_MAXDEPTH=6 OPS_CACHE_SIZE=$i >> sn_"$precsuffix"_mpi"$logical_cores"_tiled_"$i"MB_icc_diag2
	fi
	done
