...
do
	if [ -n "$CPUTEST" ]; then
	OMP_NUM_THREADS=1 mpirun -np $logical_cores $bind_hwthread ./wave-propagation_mpi 1000 1000 1000 -OPS_DIAGS=2>> so08_mpi"$logical_cores"_diag2
  if [ "$logical_cores" -ne "$physical_cores" ]; then
	OMP_NUM_THREADS=1 mpirun -np $physical_cores $bind_core ./wave-propagation_mpi 1000 1000 1000 -OPS_DIAGS=2>> so08_mpi"$physical_cores"_diag2
