...
do
	if [ -n "$CPUTEST" ]; then
	OMP_NUM_THREADS=1 mpirun -np $physical_cores -bind-to core ./opensbli_mpi 320 320 320 -OPS_DIAGS=2>> sa_"$precsuffix"_mpi"$physical_cores"_icc_diag2
  if [ "$logical_cores" -ne "$physical_cores" ]; then
	OMP_NUM_THREADS=1 mpirun -np $logical_cores -bind-to hwthread ./opensbli_mpi 320 320 320 -OPS_DIAGS=2>> sa_"$precsuffix"_mpi"$logical_cores"_icc_diag2
  ...
  if [ -n "$TILING" ]; then
	...
	for i in "${array[@]}"
	do
	OMP_NUM_THREADS=1 mpirun -np $physical_cores -bind-to core ./opensbli_mpi_tiled 320 320 320 -OPS_DIAGS=2 OPS_TILING_MAXDEPTH=12 OPS_CACHE_SIZE=$i >> sa_"$precsuffix"_mpi"$physical_cores"_tiled_"$i"MB_icc_diag2
	...
	if [ "$logical_cores" -ne "$physical_cores" ]; then
	OMP_NUM_THREADS=1 mpirun -np $logical_cores -bind-to hwthread ./opensbli_mpi_tiled 320 320 320 -OPS_DIAGS=2 OPS_TILING_MAXDEPTH=12 OPS_CACHE_SIZE=$i >> sa_"$precsuffix"_mpi"$logical_cores"_tiled_"$i"MB_icc_diag2
