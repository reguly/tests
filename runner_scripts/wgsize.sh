...
do
  ...
  do
    ...
    do
      ...
      output=$(stdbuf -o0 ./cloverleaf_"$ACCEL" -OPS_DIAGS=2 OPS_SYCL_DEVICE=$SYCL_DEVICE -gpudirect OPS_BLOCK_SIZE_X=${k} OPS_BLOCK_SIZE_Z=${j} OPS_BLOCK_SIZE_Y=${i})
      echo "$output" >> c3d_"$ACCEL"_wgtest3d
